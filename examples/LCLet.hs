...
data Exp (n :: Nat) where
    ...
    Let     :: Exp n -> (Bind Exp Exp n) -> Exp n
    -- ^ "let x = e1 in e2" where x is bound in e2
    LetRec  :: Bind Exp Exp n -> Bind Exp Exp n -> Exp n
    -- ^ "let rec x = e1 in e2" where x is bound in both e1 and e2
    LetTele :: Tele n -> Exp n
...
t4 = LetTele (LetStar t0 (bind 
              (LetStar (App (Var f0) (Var f0)) (bind
                (LetStar (App (Var f0) (Var f1)) (bind
...
instance Show (Exp n) where
    ...
    showsPrec d (Lam b) = showParen True $ 
                          showString "λ. " .
                          shows (unbind b) 
    showsPrec d (Let e1 e2) = showParen True $ 
        showString "let " . 
        showsPrec 10 e1 . 
        showString " in " .
        shows (unbind e2)
    showsPrec d (LetRec e1 e2) = showParen True $
        showString "let rec " . 
        shows (unbind e1) . 
        showString " in " .
        shows (unbind e2)
...
eval (LetRec e1 e2) = 
    ...
    let v = instantiate e2 v in
    eval (instantiate e2 v)
...
evalEnv r (App e1 e2) =
    ...
    case evalEnv r e1 of
        ...
        t -> App t v
evalEnv r (Let e1 e2) = 
    let v = evalEnv r e1 in 
    unbindWith e2 (\r' e' -> evalEnv (v .: (r' .>> r)) e')
evalEnv r (LetRec e1 e2) = 
    let v = unbindWith e1 (\r' e' -> evalEnv (v .: (r' .>> r)) e') in 
    unbindWith e2 (\r' e' -> evalEnv (v .: (r' .>> r)) e')    
...
evalTeleEnv r (LetStar e1 e2) = 
    let v = evalEnv r e1 in 
    unbindWith e2 (\r' e' -> evalTeleEnv (v .: (r' .>> r)) e')
