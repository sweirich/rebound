...
import AutoEnv.Env hiding (transform)
import AutoEnv.Env qualified as Env
...
transform :: (u -> u') -> (forall m. s m -> s' m) -> Scope u s p n -> Scope u' s' p n
transform f g (Scope u s) = Scope (Vec.map f u) (Env.transform g s)
