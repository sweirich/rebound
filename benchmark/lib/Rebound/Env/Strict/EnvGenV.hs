...
-- generic programming for implementation
-- environment argument for whnf function
-- evaluate argument before adding to environment
module Rebound.Env.Strict.EnvGenV (toDB, impl) where
...
{-# SPECIALIZE applyUnder :: SNatI n2 => (forall m n. Env DB m n -> DB m -> DB n)-> Env DB n1 n2 -> Bind DB DB n1 -> Bind DB DB n2 #-}
...
ppLC :: SNatI n => SNatI n => Int -> DB n -> Doc
