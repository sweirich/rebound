...
-- The eval function includes an explicit
-- environment argument
module Rebound.Manual.Lazy.EnvVal (toDB, impl) where
...
impl =
  LambdaImpl
    { impl_name = "Rebound.Manual.Lazy.EnvVal",
      impl_fromLC = toDB,
      impl_toLC = fromDB,
      impl_nf = error "unimplemented",
      impl_nfi = error "unimplemented",
      impl_aeq = (==),
      impl_eval = DVal . eval idE
    }
...
eval :: Env n Z -> Exp n -> Val Z
eval r e@(DVal v) = apply r v
eval r (DApp f a) =
  case eval r f of
    DLam (Bind r' b) -> eval (eval r a .: r') b
    DVar x -> case x of {}
