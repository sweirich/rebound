...
-- delay it using Bind
-- only substitutes, doesn't evaluate argument in bind
module Rebound.Manual.Lazy.Env (toDB, impl) where
...
impl =
  LambdaImpl
    { impl_name = "Rebound.Manual.Lazy.Env",
      impl_fromLC = toDB,
      impl_toLC = fromDB,
      impl_nf = nf,
