...
instance SNatI n => Eq (Exp n) where
  DVar x == DVar y = x == y
  DLam t == DLam u = t == u
...
instance SNatI a => NFData (Exp a) where
  rnf (DVar i) = rnf i
  rnf (DLam d) = rnf d
...
toDB = to []
  where
    to :: SNatI n => [(IdInt, Fin n)] -> LC IdInt -> Exp n
...
fromDB :: SNatI n => Exp n -> LC IdInt
fromDB = from firstBoundId
  where
    from :: SNatI n => IdInt -> Exp n -> LC IdInt
    from (IdInt n) (DVar i)
